                ...
                <div class="mx-auto py-4 px-4 sm:px-6 lg:px-8 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('ekstraksi.store') }}">
                        ...
                        <div>
                            ...
                            <x-text-input id="hasil_ekstraksi" class="block mt-1 w-full" type="text" name="hasil_ekstraksi" :value="old('hasil_ekstraksi')" required autofocus autocomplete="hasil_ekstraksi" />
                            <x-input-error :messages="$errors->get('hasil_ekstraksi')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="satuan_hasil" :value="__('satuan_hasil')" />
                            <x-text-input id="satuan_hasil" class="block mt-1 w-full" type="text" name="satuan_hasil" :value="old('satuan_hasil')" required autofocus autocomplete="satuan_hasil" />
                            <x-input-error :messages="$errors->get('satuan_hasil')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="tanggal_ekstraksi" :value="__('tanggal_ekstraksi')" />
                            <x-text-input id="tanggal_ekstraksi" class="block mt-1 w-full" type="text" name="tanggal_ekstraksi" :value="old('tanggal_ekstraksi')" required autofocus autocomplete="tanggal_ekstraksi" />
